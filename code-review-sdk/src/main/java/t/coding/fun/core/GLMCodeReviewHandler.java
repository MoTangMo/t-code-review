...
public class GLMCodeReviewHandler extends BaseCodeReviewHandler{
    ...
    @Override
    String getDiff() throws IOException, InterruptedException {
        return gitCommand.diff();
    }

    @Override
    String reviewCode(String codeContent) throws IOException {
        ...
        chatCompletionRequest.setMessages(new ArrayList<ChatCompletionRequestDTO.Prompt>() {
            ...
            {
                add(
                        new ChatCompletionRequestDTO.Prompt("user", "你是一位资深编程专家，拥有深厚的编程基础和广泛的技术栈知识。你的专长在于识别代码中的低效模式、安全隐患、以及可维护性问题，并能提出针对性的优化策略。你擅长以易于理解的方式解释复杂的概念，确保即使是初学者也能跟随你的指导进行有效改进。在提供优化建议时，你注重平衡性能、可读性、安全性、逻辑错误、异常处理、边界条件，以及可维护性方面的考量，同时尊重原始代码的设计意图。\n" +
                        ...
                        "{变量4}\n" +
                        "`;代码如下:"));
                add(new ChatCompletionRequestDTO.Prompt("user", codeContent));
            }
        });

        ChatCompletionSyncResponseDTO completions = openAI.handle(chatCompletionRequest);
    ...
    @Override
    String recordReview(String reviewResult) throws Exception {
